...
# ─────────────────────────────────────────────────────────────────────────────
# 4. PLOT SEPARATE CONFUSION MATRICES
# ─────────────────────────────────────────────────────────────────────────────
print("\n📊 Generating Separate Matrix Images...")

# Filter out errors
clean_df = df[(df['Affix'] != 'ERR') & (df['CRF'] != 'ERR')]

# Get all unique POS tags to ensure matching axes
labels = sorted(list(set(clean_df['Actual'].unique()) | set(clean_df['Affix'].unique()) | set(clean_df['CRF'].unique())))

# Matrix 1: Affix-Based
cm_affix = confusion_matrix(clean_df['Actual'], clean_df['Affix'], labels=labels)

plt.figure(figsize=(10, 8))
sns.heatmap(cm_affix, annot=True, fmt='d', cmap='Reds', xticklabels=labels, yticklabels=labels, cbar=False)
plt.title('Rule-Based (Affix) Confusion Matrix', fontsize=16, fontweight='bold', pad=15)
plt.xlabel('Predicted Tag', fontsize=12)
plt.ylabel('Actual Tag', fontsize=12)
plt.tight_layout()
path_affix = OUTPUT_DIR / "pos_matrix_affix.png"
plt.savefig(path_affix, dpi=300)
plt.close()
print(f"✅ Saved Affix Matrix to: {path_affix}")

# Matrix 2: CRF-Based
cm_crf = confusion_matrix(clean_df['Actual'], clean_df['CRF'], labels=labels)

plt.figure(figsize=(10, 8))
sns.heatmap(cm_crf, annot=True, fmt='d', cmap='Greens', xticklabels=labels, yticklabels=labels, cbar=False)
plt.title('Hybrid (CRF) Confusion Matrix', fontsize=16, fontweight='bold', pad=15)
plt.xlabel('Predicted Tag', fontsize=12)
plt.ylabel('Actual Tag', fontsize=12)
plt.tight_layout()
path_crf = OUTPUT_DIR / "pos_matrix_crf.png"
plt.savefig(path_crf, dpi=300)
plt.close()
print(f"✅ Saved CRF Matrix to:   {path_crf}")