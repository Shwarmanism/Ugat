...
if __name__ == "__main__":
    verifier = DialectVerifier()
    
    # Using your "Complex/Hard" Test Cases to stress-test the verifier
    test_cases = [
        # ---------------------------------------------------------
        # ILOCANO CASES (Should Match)
        # ---------------------------------------------------------
        ("Ag-agawid kami no malpas ti trabaho", "ilocano"),      # Strong: ag-, ti
        ("Ginatangna ti dakkel a tinapay", "ilocano"),           # Strong: ti
        ("Kayatko nga agbasa ti libro", "ilocano"),              # Strong: ag-, ti, nga

        # ---------------------------------------------------------
        # HILIGAYNON CASES (Should Match)
        # ---------------------------------------------------------
        ("Nagalakat-lakat kami sa plaza", "hiligaynon"),         # Strong: naga-
        ("Guapa-guapa gid ang bata ni Maria", "hiligaynon"),     # Strong: gid
        ("Nagahimo sila sang balay-balay", "hiligaynon"),        # Strong: naga-, sang

        # ---------------------------------------------------------
        # CEBUANO CASES (Should Match)
        # ---------------------------------------------------------
        ("Mokaon ang bata og tinapay", "cebuano"),               # Strong: mo-, og
        ("Gipahibalo sa kapitan ang balita", "cebuano"),         # Strong: gi-
        ("Naglantaw-lantaw siya sa dagat", "cebuano"),           # Strong: nag-, sa (Shared but context helps)

        # ---------------------------------------------------------
        # MISMATCH CASES (Should be False)
        # ---------------------------------------------------------
        ("Ag-agawid kami no malpas ti trabaho", "cebuano"),      # Fail: Ilocano markers
        ("Guapa-guapa gid ang bata", "cebuano"),                 # Fail: 'gid' is Hiligaynon
        ("Mokaon ang bata og tinapay", "hiligaynon")             # Fail: 'mo-', 'og' are Cebuano
    ]
    
    print(f"{'INPUT':<40} | {'SELECTED':<12} | {'CONF':<5} | {'DETECTED':<12} | {'MATCH?'}")
    print("-" * 95)
    
    for text, dialect in test_cases:
        res = verifier.get_confidence(text, dialect)
        
        # Color code the output for easy reading
        match_icon = "✅" if res['is_match'] else "❌"
        
        print(f"{text:<40} | {dialect:<12} | {res['confidence']:<5} | {res['detected']:<12} | {match_icon} {res['is_match']}")